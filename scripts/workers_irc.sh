...
for f in "${filenames[@]}"
do
  osascript -e 'tell application "Terminal"' -e 'tell application "System Events" to keystroke "t" using {command down}' -e 'do script "cd '"$d"'; python3 '"$f"'" in selected tab of the front window' -e 'end tell'
done
